...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByRole('button', {name: 'Shared read', exact: true}).getAttribute('aria-pressed'), 'true');
    ...
    assert.equal(await page.getByRole('button', {name: 'Shared read', exact: true}).getAttribute('aria-pressed'), 'false');
    assert.equal(await page.getByRole('button', {name: 'One substitution', exact: true}).getAttribute('aria-pressed'), 'true');
    ...
    assert.equal(await page.getByRole('button', {name: 'Unique read', exact: true}).getAttribute('aria-pressed'), 'true');
    assert.match(await content('k-read-result'), /\{T1\}/);
