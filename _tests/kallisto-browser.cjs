...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.k-equation[data-math-rendered]').count(), 6, 'All display equations render');
    ...
    assert.equal(await page.locator('.k-paper-figure img').count(), 4, 'Original paper figures are embedded');
    assert.equal(await page.locator('.k-try').count(), 5, 'Every Try it paragraph is a callout');
    assert.equal(await page.locator('.k-try > p > strong:first-child').evaluateAll(labels => labels.every(label => label.textContent.trim() === 'Try it:')), true, 'Try it callouts have a consistent label');
    ...
    assert.ok(await page.locator('#kallisto-graph-overview img').evaluate(img => img.getBoundingClientRect().width <= 500), 'Graph overview stays compact in the article');
    await page.locator('#kallisto-graph-overview').screenshot({path: path.join(os.tmpdir(), 'kallisto-graph-overview.png')});
    ...
    assert.equal(await page.getByRole('button', {name: /Shared read/}).getAttribute('aria-pressed'), 'true');
    assert.match(await content('k-example-description'), /complete sequence occurs in T1 and T3/);
    ...
    await page.getByRole('button', {name: /One substitution/}).click();
    assert.equal(await page.getByRole('button', {name: /Shared read/}).getAttribute('aria-pressed'), 'false');
    assert.equal(await page.getByRole('button', {name: /One substitution/}).getAttribute('aria-pressed'), 'true');
    assert.match(await content('k-example-description'), /one base differs from the T1 segment/);
    ...
    await page.getByRole('button', {name: /No matches/}).click();
    assert.match(await content('k-sample-totals'), /260 assigned \+ 0 unassigned/);
    assert.deepEqual(await page.locator('#k-read-matrix tr').first().locator('[data-compatible]').evaluateAll(cells => cells.map(cell => cell.dataset.compatible)), ['true', 'false', 'false']);
    assert.notEqual(await page.locator('#k-read-matrix tr').first().locator('[data-compatible]').nth(0).evaluate(cell => getComputedStyle(cell).backgroundColor), await page.locator('#k-read-matrix tr').first().locator('[data-compatible]').nth(1).evaluate(cell => getComputedStyle(cell).backgroundColor), 'Compatible cells have a distinct background');
    await page.locator('#k-class-size').fill('7');
    assert.equal(await page.locator('#k-size').inputValue(), '7');
    assert.match(await content('k-sample-totals'), /140 assigned \+ 120 unassigned/);
    await page.locator('#k-reset-reads').click();
    assert.equal(await page.locator('#k-class-size').inputValue(), '3');
    assert.equal(await page.locator('#k-size').inputValue(), '3');
    await page.locator('[data-count="0"]').fill('200');
    await page.locator('[data-count="0"]').press('Tab');
    assert.match(await content('k-sample-totals'), /360 assigned \+ 0 unassigned/);
    await page.locator('#k-reset-reads').click();
    assert.match(await content('k-sample-totals'), /260 assigned \+ 0 unassigned/);
    await page.locator('#class-lab').screenshot({path: path.join(os.tmpdir(), 'kallisto-classes.png')});
    ...
    assert.equal(await page.getByRole('button', {name: /Unique read/}).getAttribute('aria-pressed'), 'true');
    assert.match(await content('k-example-description'), /combining them leaves only T1 compatible/);
    assert.match(await content('k-read-result'), /\{T1\}/);
    ...
    await page.locator('#k-em-animate').click();
    assert.equal(await page.locator('#k-em-animate').getAttribute('aria-pressed'), 'true');
    await page.waitForSelector('#k-em-phase-e[data-active="true"]');
    const chartTopDuringE = await page.locator('#k-em-chart').evaluate(chart => chart.getBoundingClientRect().top);
    await page.locator('#k-em-cycle').screenshot({path: path.join(os.tmpdir(), 'kallisto-em-animation.png')});
    await page.waitForSelector('#k-em-phase-m[data-active="true"]');
    const chartTopDuringM = await page.locator('#k-em-chart').evaluate(chart => chart.getBoundingClientRect().top);
    assert.ok(Math.abs(chartTopDuringE - chartTopDuringM) < 1, 'EM trajectory stays fixed between E-step and M-step');
    await page.waitForFunction(() => document.querySelector('#k-em-status').textContent.includes('Animation complete: converged'), null, {timeout: 20000});
    assert.equal(await page.locator('#k-em-animate').getAttribute('aria-pressed'), 'false');
    ...
    for (let i = 0; i < await page.locator('[data-count]').count(); i++) {
      await page.locator('[data-count]').nth(i).fill('0');
      await page.locator('[data-count]').nth(i).press('Tab');
    }
    ...
    assert.deepEqual(await page.locator('h2[id]').evaluateAll(headings => headings.map(heading => heading.id)), ['the-problem', 'k-mers', 'compatibility-classes', 'effective-length', 'em', 'neural-network', 'gpu', 'bowtie2-rsem', 'conclusion', 'references']);
    assert.deepEqual(await page.locator('.k-roadmap a').evaluateAll(links => links.map(link => link.getAttribute('href'))), ['#the-problem', '#k-mers', '#sequence-lab', '#compatibility-classes', '#class-lab', '#effective-length', '#em', '#em-lab', '#neural-network', '#gpu', '#bowtie2-rsem', '#conclusion', '#references']);
    assert.equal(await page.locator('.k-references > li').count(), 10, 'Every cited source appears in the reference list');
    ...
    console.log('Browser checks passed: LaTeX rendering and MathML, local math assets, original paper images, all three labs, custom sample transfer, exact EM steps, degeneracy, slow convergence, invalid/empty states, keyboard operation, mobile overflow, no-JS content, and homepage isolation.');
